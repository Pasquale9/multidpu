...
#define DEVICE_NAME_0 "multidpu0"
#define DEVICE_NAME_1 "multidpu1"
#define DRV_NAME "unina-multidpu"
 ...
 */
 ...
 uint32_t device_count = 0; // variabile per capire di quale DPU si tratta
...
{	printk("\n\n\n\n\n SONO MULTI NELLA __func__\n\n\n\n\n");
	...
	int i, ret;
	u32 val;
	struct xdpu_dev *xdpu_1;
	struct resource *res;
	struct device *dev = &pdev->dev;

	 xdpu_1 = devm_kzalloc(dev, sizeof(*xdpu_1), GFP_KERNEL);
	if (!xdpu_1){
		printk("\n\n\n\n\n Sono nell'if !xdpu \n\n\n\n\n");
		return -ENOMEM;
	}
	...
	xdpu_1->regs = devm_platform_get_and_ioremap_resource(pdev, 0, &res);
	if (IS_ERR(xdpu_1->regs)){
		printk("\n\n\n\n\n Sono nell'if IS_ERR(xdpu->regs) \n\n\n\n\n");
		return -ENOMEM;
	}
	 ...
	 */
	xdpu_1->axi_clk = devm_clk_get_optional(xdpu_1->dev, "s_axi_aclk");
	if (IS_ERR(xdpu_1->axi_clk))
		return dev_err_probe(xdpu_1->dev, PTR_ERR(xdpu_1->axi_clk),
				     "unable to get axi reference clock\n");

	ret = clk_prepare_enable(xdpu_1->axi_clk);
	if (ret) {
		dev_err(xdpu_1->dev, "failed to enable s_axi_aclk(%d)\n", ret);
		return ret;
	}

	xdpu_1->dpu_clk = devm_clk_get_optional(xdpu_1->dev, "m_axi_dpu_aclk");
	if (IS_ERR(xdpu_1->dpu_clk))
		return dev_err_probe(xdpu_1->dev, PTR_ERR(xdpu_1->dpu_clk),
				     "unable to get m_axi_dpu_aclk\n");

	ret = clk_prepare_enable(xdpu_1->dpu_clk);
	if (ret) {
		dev_err(xdpu_1->dev, "unable to enable dpu_clk(%d)\n", ret);
		goto err_dpuclk;
	}

	xdpu_1->dsp_clk = devm_clk_get_optional(xdpu_1->dev, "dpu_2x_clk");
	if (IS_ERR(xdpu_1->dsp_clk))
		return dev_err_probe(xdpu_1->dev, PTR_ERR(xdpu_1->dsp_clk),
				     "unable to get dsp clock\n");

	ret = clk_prepare_enable(xdpu_1->dsp_clk);
	if (ret) {
		dev_err(xdpu_1->dev, "unable to enable dpu_2x_clk(%d)\n", ret);
		goto err_dspclk;
	}

	/* dsp_clk should be dpu_clk * 2 */
	if (xdpu_1->axi_clk && xdpu_1->dpu_clk && xdpu_1->dsp_clk)
		dev_dbg(xdpu_1->dev,
			"Freq: axilite: %lu MHz, dpu: %lu MHz, dsp: %lu MHz",
			clk_get_rate(xdpu_1->axi_clk) / 1000000,
			clk_get_rate(xdpu_1->dpu_clk) / 1000000,
			clk_get_rate(xdpu_1->dsp_clk) / 1000000);

	val = ioread32(xdpu_1->regs + DPU_IPVER_INFO);
	if (DPU_VER(val) < DPU_IP_V3_4) {
		dev_err(dev, "DPU IP need upgrade to 3.4 or later");
		goto err_out;
	}

	xdpu_1->dpu_cnt = DPU_NUM(val);
	xdpu_1->sfm_cnt = SFM_NUM(val);

	val = ioread32(xdpu_1->regs + DPU_IPFREQENCY);
	dev_dbg(dev, "found %d dpu core @%ldMHz and %d softmax core",
		xdpu_1->dpu_cnt, DPU_FREQ(val), xdpu_1->sfm_cnt);

	if (get_irq(pdev, xdpu_1))
		goto err_out;
	...
	mutex_init(&xdpu_1->mutex);

	for (i = 0; i < xdpu_1->dpu_cnt + xdpu_1->sfm_cnt; i++) {
		init_completion(&xdpu_1->cu[i].done);
		mutex_init(&xdpu_1->cu[i].mutex);
	}
	printk ("device_count = %d", device_count);

		printk ("inizializzo la DPU0");
		xdpu_1->miscdev.minor = MISC_DYNAMIC_MINOR;
		xdpu_1->miscdev.name = DEVICE_NAME_0;
		xdpu_1->miscdev.fops = &dev_fops;
		xdpu_1->miscdev.parent = dev;
	

	device_count++;
	printk ("device_count = %d", device_count);
	int out_misc_register=misc_register(&xdpu_1->miscdev);
	printk ("misc_register = %d", out_misc_register);
	if (out_misc_register)
		goto err_out;

	xlnx_dpu_regs_init(xdpu_1);

#ifdef CONFIG_DEBUG_FS
	INIT_LIST_HEAD(&xdpu_1->client_list);

	ret = dpu_debugfs_init(xdpu_1);
	if (ret) {
		dev_err(xdpu_1->dev, "failed to init dpu_debugfs)\n");
		goto err_debugfs;
	}
#endif

	platform_set_drvdata(pdev, xdpu_1);

	dev_dbg(dev, "dpu registered as /dev/multidpu0 successfully");

	return 0;

#ifdef CONFIG_DEBUG_FS
err_debugfs:
	misc_deregister(&xdpu_1->miscdev);
#endif
err_out:
	clk_disable_unprepare(xdpu_1->dsp_clk);
err_dspclk:
	clk_disable_unprepare(xdpu_1->dpu_clk);
err_dpuclk:
	clk_disable_unprepare(xdpu_1->axi_clk);
	...
	printk ("device_count = %d", device_count);
		...
		printk ("inizializzo la DPU1");
		xdpu->miscdev.minor = MISC_DYNAMIC_MINOR;
		xdpu->miscdev.name = DEVICE_NAME_1;
	...
	device_count++;
	printk ("device_count = %d", device_count);
	int out_misc_register=misc_register(&xdpu->miscdev);
	printk ("misc_register = %d", out_misc_register);
	if (out_misc_register)
		goto err_out;
	...
	dev_dbg(dev, "dpu registered as /dev/multidpu1 successfully");
...
#endif
	platform_set_drvdata(pdev, NULL); //controllare cosa fa questa funzione sia qui che nella probe
	misc_deregister(&xdpu->miscdev);
	device_count--;
	dev_dbg(xdpu->dev, "%s: device /dev/dpu unregistered\n", __func__);//camvbiare l'output per capire meglio il debug 
	return 0;
}

static const struct of_device_id dpu_of_match[] = { //da cambiare le stringhe
	{ .compatible = "unina,multidpu" },
	{ /* end of table */ }
};
...
static int dpu_debugfs_init(struct xdpu_dev *xdpu)
{
	...
	int i;
	if(device_count==0){
	xdpu->root = debugfs_create_dir("multidpu", NULL);
	if (IS_ERR(xdpu->root)) {
